...
export const PromptInputBox = React.forwardRef((props, ref) => {
  ...
  const [recordedVoiceData, setRecordedVoiceData] = React.useState(null);
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0 || recordedVoiceData) {
      onSend(input, files, recordedVoiceData);
      setInput("");
      setFiles([]);
      setFilePreviews({});
  ...
  const handleStopRecording = (duration, audioBlob) => {
    console.log(`Stopped recording after ${duration} seconds`);
    setIsRecording(false);
    if (audioBlob) {
      setRecordedVoiceData(audioBlob);
    } else {
      console.error("No audio blob received from VoiceRecorder.");
    }
  };

  const hasContent = input.trim() !== "" || files.length > 0 || recordedVoiceData;
  ...
  return (
    <>
      <PromptInput
        ...
        {/* File/Voice Preview Section - Above input */}
        {(files.length > 0 || recordedVoiceData) && (
          <div className="flex flex-wrap gap-2 p-0 pb-2 transition-all duration-300">
            ...
            {recordedVoiceData && (
              <div className="flex items-center gap-2 bg-[#2E3033] rounded-xl px-3 py-2">
                <audio controls src={URL.createObjectURL(recordedVoiceData)} className="h-8" />
                <button
                  onClick={() => setRecordedVoiceData(null)}
                  className="rounded-full bg-black/70 p-1 hover:bg-black transition-colors">
                  <X className="h-3 w-3 text-white" />
                </button>
              </div>
            )}
        ...
        {/* Main Input Row - File Upload + Text Input + Voice/Send Button */}
        <div className="flex items-center gap-2">
          {/* File Upload Button */}
          <PromptInputAction tooltip="Upload image">
            <button
              onClick={() => uploadInputRef.current?.click()}
              className="flex h-8 w-8 shrink-0 text-[#9CA3AF] cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-gray-600/30 hover:text-[#D1D5DB]"
